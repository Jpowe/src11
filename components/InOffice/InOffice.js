...
class InOffice extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    this.setState({
      data: nextProps.data
      //  topRow: nextProps.selfRow
    });
    */
    this.setState(prevState => {
      return { data: prevState.data + nextProps.data };
    });
    this.setState({
      topRow: R.find(x => x.uuid === this.getSelectedUUID(), nextProps.data)
    });
    this.tick(nextProps.data);
  }
  ...
  tick = data => {
    ...
    data.map((x, i) => {
      ...
      //  console.log(x.firstName + " " + date + " " + utcDate);
      if (date - utcDate) {
        //console.log("here a1 " + x.firstName + " " + date);
        /* previous days */
        x.returnMessage = null;
      } else {
        //  console.log("here b1 " + date);
        if (hr > 16) {
